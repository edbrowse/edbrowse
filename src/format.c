...
// if you run this through indent, it formats this array
// of structs, or any array of structs, in a way I don't like.
static const struct reservedColor {
	const char *name;
	uchar r, g, b;
} colorlist[] = {
...
{
	const struct reservedColor *c, *best_c = 0;
	int best_val;
	int r1, g1, b1;
	const char *t;
...
// convert color keyword back to rgb
const char *color2rgb(const char *word)
{
	static char rgb_string[20];
	const struct reservedColor *c;
	const char *s;
	if(!word) // should never happen
		return emptyString;
	for(s = word; *s; ++s)
