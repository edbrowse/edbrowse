...
// for pathname resolution, we need / at the end
				j = strlen(v);
				if(v[j - 1] != '/') {
					v = reallocMem(v, j + 2);
					strcpy(v + j, "/");
				}
			} else if(!fileIntoMemory(v, &incbuf, &inclen, 0)) {
				showError();
				setError(-1);
				nzFree(v);
				continue;
